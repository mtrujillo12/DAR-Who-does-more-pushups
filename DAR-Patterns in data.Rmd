...
We will be using a boxplot and a numerical dataset to analyze the variables.
...
```{r}
a <- ggplot(pushups,aes(x=position,y=pushups))
a + geom_boxplot()
```
...
This numerical dataset shows that the skill players did more pushups than line players.
...
We are testing to see if skill players do more pushups than line players. 
```{r}
t.test(pushups~position,data=pushups)
```

The mean for the line player group is 17.13 while the mean for the skill player group is 49.72, having a confidence interval of (-44.91,-20.3).
...
The Skills group was significantly better at doing push-ups than the line player group. We get this result with help from the box plot, the numerical dataset, and the t-test.